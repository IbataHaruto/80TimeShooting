...
public class PlayerMovement : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        //  しゃがみ中は Movement は状態を送らない 
        if (PlayerStateManager.Instance.CurrentState == PlayerState.Crouch)
            return;

        // --- ダッシュON かつ 移動中 のときだけ Dash ---
        if (isDashToggled && isMoving)
            PlayerStateManager.Instance.SetState(PlayerState.Dash);
        else
